	...
	switch(chl)
	{
		case 1:
			...
			if( temp > Sys_Para_T.TH_1_Warning[0] && Sys_Para_T.TH_1_Warning[0]!=0 )  /*1路温度大于上限值，且阈值不为0(为0时不启用报警)*/
			{
				...
				SET_RELAY(1);                        //温度超过上限 开启继电器 切换阀门
				DIS_Suona(1,1);
				DIS_TOPDOWN(1,1);
				BACKLIGHT_SET(Sys_Para_T.BackLightMode,1,2);
			}
			
			else if( temp < Sys_Para_T.TH_1_Warning[1] && Sys_Para_T.TH_1_Warning[1]!=0 )  /*1路温度小于下限值，且阈值不为0(为0时不启用报警)*/
			{
				BEEP_Speek(SpeekTime,SpeekFre);
				SET_RELAY(1);                        //温度超过下限 开启继电器 切换阀门
				DIS_Suona(1,1);
				DIS_TOPDOWN(1,2);
			...
			else if( humi < Sys_Para_T.TH_1_Warning[3] && Sys_Para_T.TH_1_Warning[3]!=0 )  /*1路湿度大于上限值，且阈值不为0(为0时不启用报警)*/
			{
				BEEP_Speek(SpeekTime,SpeekFre);
				DIS_Suona(2,1);
				DIS_TOPDOWN(2,2);
		...
		case 2:
			...
			else if( temp < Sys_Para_T.TH_2_Warning[1] && Sys_Para_T.TH_2_Warning[1]!=0 )  /*2路温度小于下限值，且阈值不为0(为0时不启用报警)*/
			{
				BEEP_Speek(SpeekTime,SpeekFre);
				SET_RELAY(0);                        //温度超过下限 关闭继电器 切换阀门
				DIS_Suona(1,1);
				DIS_TOPDOWN(1,2);
				BACKLIGHT_SET(Sys_Para_T.BackLightMode,2,1);
			}
			...
			else if( humi < Sys_Para_T.TH_2_Warning[3] && Sys_Para_T.TH_2_Warning[3]!=0 )  /*2路湿度大于上限值，且阈值不为0(为0时不启用报警)*/
			{
				BEEP_Speek(SpeekTime,SpeekFre);
				DIS_Suona(2,1);
				DIS_TOPDOWN(2,2);
